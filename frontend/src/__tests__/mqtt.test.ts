...
describe("MQTTClient", () => {
    ...
    it("publish() sends JSON and returns true when connected", () => {
        ...
        expect(ok).toBe(true);
        expect(mockMqttClient.publish).toHaveBeenCalledWith("io/chat", JSON.stringify({ content: "hello" }), {
            qos: 1,
        });
    });
    ...
    it("off() removes a specific listener", () => {
        ...
        triggerMessage("agents/id/heartbeat", {
            agentId: "x",
            agentName: "x",
            state: "running",
            sequence: 0,
    ...
    it("routes agents/{id}/chat → 'chat'", () => {
        const spy = vi.fn();
        client.on("chat", spy);
        triggerMessage("agents/abc/chat", {
            id: "m1",
            from: "abc",
            to: "user",
            content: "hi",
    ...
    it("routes agents/{id}/spawn → 'spawn'", () => {
        const spy = vi.fn();
        client.on("spawn", spy);
        triggerMessage("agents/abc/spawn", {
            agentId: "abc",
            agentName: "abc",
            agentType: "dynamic",
            timestampMs: Date.now(),
