...
class MainActor(LLMAgent):
    ...
    # ── LLM agent ↔ remote runner bridging ────────────────────────────────────
    # A `type: "llm"` agent has no `code` — its behaviour is implemented inside
    # the LLMAgent class (which lives only on main). When such an agent runs on
    # a remote node, the runner is DynamicAgent-shaped only; it has no LLMAgent
    # equivalent. To make LLM agents portable, we synthesise an equivalent
    # `handle_task` that runs in the remote runner and reproduces the four
    # essentials of LLMAgent: conversation memory, persistence per turn, cost
    # tracking via the LLM bridge, and a {"text": ...} reply shape.

    _REMOTE_LLM_AGENT_CODE = '''\
# Auto-generated by MainActor for a `type: "llm"` agent running remotely.
# Mirrors llm_agent.LLMAgent._handle_task at a behavioural level so the
# same agent feels identical to the user whether local or remote.

_SYSTEM_PROMPT = """__SYSTEM_PROMPT__"""
_MAX_HISTORY  = __MAX_HISTORY__

async def setup(agent):
    # Restore conversation history persisted by an earlier incarnation —
    # either on this node from a previous runner restart, or from a
    # migration `_initial_state` snapshot applied by the remote runner.
    agent.state["history"] = list(agent.recall("conversation_history", []) or [])
    agent.state["summary"] = agent.recall("history_summary", "") or ""
    if agent.state["history"]:
        await agent.log(
            f"Restored {len(agent.state['history'])} turn(s) of history",
            level="info",
        )

async def handle_task(agent, payload):
    # Accept text under several common keys to match LLMAgent's tolerance.
    if isinstance(payload, dict):
    ...
    # Trim — keep the contract identical to LLMAgent.max_history.
    safe = [m for m in history[-_MAX_HISTORY:]
            if isinstance(m, dict) and m.get("role") in ("user", "assistant")]

    # The bridge call (agent.llm.complete) routes through main and never
    # exposes the API key here on the edge.
    reply = await agent.llm.complete(messages=safe, system=_SYSTEM_PROMPT)

    history.append({"role": "assistant", "content": reply})
    agent.state["history"] = history

    # Persist after EVERY exchange so a crash mid-conversation doesn't lose
    # turns. `agent.persist` on the remote writes to the agent's JSON state
    # file synchronously — same durability the local LLMAgent gets via its
    # PersistenceAPI.
...
'''
    ...
    def _synthesise_llm_agent_code(self, system_prompt: str,
                                   max_history: int = 24) -> str:
        """Return the auto-generated DynamicAgent-style code that gives a
        `type: "llm"` agent its full LLMAgent-equivalent behaviour on a
        remote node."""
        safe_prompt = (system_prompt or "You are a helpful assistant.")
        # Escape triple-quotes — the prompt becomes a Python triple-quoted
        # string literal in the synthesised module. Backslashes are also
        # doubled so JSON-escaped sequences inside the prompt survive.
        safe_prompt = safe_prompt.replace("\\", "\\\\").replace('"""', '\\"\\"\\"')
        code = self._REMOTE_LLM_AGENT_CODE.replace(
            "__SYSTEM_PROMPT__", safe_prompt
        ).replace(
            "__MAX_HISTORY__", str(int(max_history))
        )
        return code

    def _is_llm_only_config(self, config: dict) -> bool:
        """True if the config describes an LLMAgent-style spawn — no user
        code, just a system_prompt and (optionally) type=llm."""
        ...
        if config.get("type") == "llm":
            return True
        # Heuristic: a system_prompt with no code is effectively an LLM agent.
        return bool((config.get("system_prompt") or "").strip())

    def _upgrade_config_for_remote_llm(self, config: dict) -> dict:
        """If `config` is an LLM-only spawn going to a remote node, attach
        synthesised code so the remote runner can actually run it. Returns
        a new dict; the input is not mutated.

        No-op for configs that already have `code` (DynamicAgent style) or
        that aren't LLM-shaped at all (e.g. ha_actuator, scheduled, manual)."""
        if not self._is_llm_only_config(config):
            return config
        upgraded = dict(config)
        upgraded["code"] = self._synthesise_llm_agent_code(
            system_prompt=config.get("system_prompt", ""),
            max_history=int(config.get("max_history", 24)),
        )
        # Preserve the original intent in the config so the spawn registry
        # remembers this is an LLM agent — useful for migrating it back to
        # local where the LLMAgent class should be used instead of the
        # synthesised code.
        upgraded["_origin_type"] = "llm"
        return upgraded

    def _apply_initial_state_locally(self, name: str, initial_state: dict):
        """Write a migration `_initial_state` snapshot into the local agent's
        persistence, so the next `recall(...)` call returns the migrated value.

        Called after a remote→local spawn so things like conversation_history
        and token totals survive the trip back. The agent's PersistenceAPI is
        the source of truth on the local side; the in-memory _persistent_state
        is also updated so an immediate recall() before the agent's setup()
        runs gets the right value.
        """
        ...
        if actor is None:
            logger.warning(
                f"[{self.name}] _apply_initial_state_locally('{name}'): "
                f"actor not found in registry; skipping"
            )
            return
        try:
            # Mirror Actor.persist() semantics: write through PersistenceAPI
            # AND update the in-memory dict so the next recall() sees it.
            for k, v in initial_state.items():
                if hasattr(actor, "_persistence_api") and actor._persistence_api:
                    try:
                        actor._persistence_api.set(k, v)
                    except Exception as e:
                        logger.debug(f"[{self.name}] persist({k}) on '{name}' failed: {e}")
                if hasattr(actor, "_persistent_state") and isinstance(
                        actor._persistent_state, dict):
                    actor._persistent_state[k] = v
            logger.info(
                f"[{self.name}] Applied {len(initial_state)} migrated state key(s) "
                f"to local agent '{name}': {list(initial_state.keys())}"
            )
            # LLMAgent caches some fields in instance attributes (e.g.
            # _conversation_history). Refresh them from the just-written
            # persistence so the next message sees the migrated turns.
            ch = initial_state.get("conversation_history")
            if ch is not None and hasattr(actor, "_conversation_history"):
                actor._conversation_history = list(ch)
            hs = initial_state.get("history_summary")
            if hs is not None and hasattr(actor, "_history_summary"):
                actor._history_summary = hs
            fc = initial_state.get("_final_cost")
            if isinstance(fc, dict):
                if hasattr(actor, "total_input_tokens"):
                    actor.total_input_tokens  = fc.get("input_tokens",
                                                       actor.total_input_tokens)
                if hasattr(actor, "total_output_tokens"):
                    actor.total_output_tokens = fc.get("output_tokens",
                                                       actor.total_output_tokens)
                if hasattr(actor, "total_cost_usd"):
                    actor.total_cost_usd      = fc.get("cost_usd",
                                                       actor.total_cost_usd)
        except Exception as e:
            logger.warning(
                f"[{self.name}] _apply_initial_state_locally('{name}') failed: {e}"
            )
    ...
    async def _spawn_remote(self, config: dict, node: str, save: bool) -> None:
        ...
        """
        # ── LLM agent → remote-runnable code ──────────────────────────────────
        # The LLMAgent class lives only on main. If we ship a `type: "llm"`
        # config (system_prompt only, no code) to a remote runner, it has no
        # `handle_task` to call and @mentions silently fail. Synthesise an
        # equivalent so the same agent behaves the same on both sides — and
        # remember it was originally LLM-typed so migration back to local can
        # restore the proper LLMAgent class.
        config = self._upgrade_config_for_remote_llm(config)
            ...
            try:
                async with aiomqtt.Client(self._mqtt_broker, self._mqtt_port) as client:
                    ...
                    async for msg in client.messages:
                        ...
                        # Restore LLMAgent shape if this agent was originally
                        # a `type: "llm"` config that we upgraded with synth-
                        # esised code for the remote runner. Locally, the
                        # real LLMAgent class is better — it has summarisation,
                        # metrics, and cost persistence the synthesised code
                        # doesn't fully replicate.
                        if local_cfg.pop("_origin_type", None) == "llm":
                            local_cfg["type"] = "llm"
                        ...
                        try:
                            ...
                            # Apply migrated state to the just-spawned agent's
                            # persistence so recall(...) inside setup() returns
                            # the migrated values. _spawn_from_config does not
                            # consume `_initial_state` on the local side — it's
                            # only meaningful to the remote runner — so we
                            # explicitly mirror that semantics here.
                            self._apply_initial_state_locally(agent_name, state)
    ...
    async def migrate_agent(self, agent_name: str, target_node: str) -> dict:
        ...
        if current_node and not is_target_local:
            ...
            # Always use the '@main' sentinel so the remote node ships back
            # its LIVE config + state. The previous "have_code" optimisation
            # used the spawn-registry config, which is the config we sent at
            # spawn time — not what the agent had learned since (new
            # conversation turns, observed schemas, restored history). That
            # silently lost state on every remote→local migration. The extra
            # MQTT round-trip is cheap; correctness wins.
            logger.info(
                f"[{self.name}] Migrating '{agent_name}' from node "
                f"'{current_node}' → local (via @main sentinel for live state)"
            )

            import secrets
            return_token = secrets.token_hex(8)
            # Stash the token so the listener knows this return is ours and
            # not from some other concurrent migration.
