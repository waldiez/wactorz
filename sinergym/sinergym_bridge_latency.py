...
# Round-trip latency instrumentation (kept in its own module).
from latency_meter import LatencyMeter
...
class SinergymBridgeMAS:
    def __init__(
        ...
        # Only meaningful when an external agent actually answers, i.e. NOT in
        # 'collect' mode (there actions are sampled locally, so the loop never
        # leaves this process). Warn and disable if asked for in collect mode.
        _lat_on = measure_latency
        if measure_latency and mode == "collect":
            print("[LATENCY] --measure-latency ignored in 'collect' mode "
                  "(no external agent answers; use --mode deploy).")
            _lat_on = False
    ...
    def _run_episode(self, env: gym.Env, ep: int) -> dict:
        ...
        while not done:
            _fb_before = self._fallback_steps
            action_real, action_norm = self._get_action(env)
            # Stop the latency clock the moment the bridge has the action in hand.
            # Only a genuine reply counts: if _get_action fell back to a default
            # (agent timed out), _fallback_steps grew — discard that sample.
            self._latency.mark_recv(valid=(self._fallback_steps == _fb_before))
            ...
            # Start the latency clock right before the obs leaves the bridge.
            # Every agent (single-agent and per-zone fleet) subscribes to this
            # global obs topic, so this is the trigger for the next action.
            self._latency.mark_send()
...
Examples:
  ...
  python sinergym_bridge_mas.py --env Eplus-5zone-hot-continuous-v1 --episodes 10 \\
    ...
    p.add_argument("--measure-latency", action="store_true",
                   help="Measure obs→action round-trip latency (deploy/eval only). "
                        "Times from just before publishing the observation to just "
                        "after the action is received, all on this device.")
    p.add_argument("--latency-count",  type=int, default=100,
                   help="Number of valid round-trip samples to collect before "
                        "reporting the mean (default 100).")
    p.add_argument("--latency-warmup", type=int, default=10,
                   help="Initial samples to discard before averaging, to skip "
                        "cold-start effects (default 10).")
    p.add_argument("--latency-label",  default=None,
                   help="Label for the latency report (default: this hostname). "
                        "Handy to tell a laptop-bridge run from an rpi5-bridge run.")
    p.add_argument("--latency-csv",    default=None,
                   help="Optional path to dump the raw per-step latency samples "
                        "as CSV (off by default).")
    return p.parse_args()
