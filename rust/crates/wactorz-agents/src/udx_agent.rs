...
impl UdxAgent {
    ...
    fn cmd_explain(concept: &str) -> String {
        match concept {
            ...
            "rust" => "\
                **Rust backend**\n\
                The server is a single `wactorz` binary built with Tokio async runtime.\n\
                Workspace crates:\n\
                `wactorz-core`       → Actor trait, registry, message types, publisher\n\
                `wactorz-agents`     → all concrete agent implementations\n\
                `wactorz-mqtt`       → MQTT client + topic helpers\n\
                `wactorz-interfaces` → REST (axum) + WebSocket bridge + CLI\n\
                `wactorz-server`     → binary entry point, wires everything together\n\n\
                Build: `cargo build --release --bin wactorz`\n\
    ...
    fn cmd_version() -> String {
        format!(
            "**AgentFlow** · Rust {rust} · built {built}\n\
             Backend crates: `wactorz-core` · `wactorz-agents` · `wactorz-mqtt` · \
    ...
    fn doc_deploy() -> String {
        ...
        **Native binary** (`compose.native.yaml`):\n\
        `bash scripts/package-native.sh`   — builds `wactorz-native-*.tar.gz`\n\
