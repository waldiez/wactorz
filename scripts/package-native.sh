#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Wactorz — native-binary release packager
#
# Produces:  wactorz-native-<YYYYMMDD>.tar.gz  (~20 MB vs 44 MB full zip)
#
# Contents (NO Docker image — binary is run directly on the host):
#   wactorz               stripped linux/amd64 binary
...
DATE=$(date +%Y%m%d)
RELEASE_NAME="wactorz-native-${DATE}"
WORK_DIR="/tmp/${RELEASE_NAME}"
OUT_FILE="${RELEASE_NAME}.tar.gz"
BINARY="wactorz"

echo "══════════════════════════════════════════════════════"
echo " Wactorz Native-Binary Packager"
...
docker buildx build \
  ...
  --tag wactorz-server:native-extract \
  --load \
  ./rust

echo ""
echo "▶ Extracting binary from image…"
CONTAINER_ID=$(docker create --platform linux/amd64 wactorz-server:native-extract)
docker cp "${CONTAINER_ID}:/app/${BINARY}" "/tmp/${BINARY}-linux-amd64"
docker rm  "${CONTAINER_ID}" >/dev/null
docker rmi wactorz-server:native-extract --force >/dev/null 2>&1 || true
...
#!/usr/bin/env bash
# Wactorz — native-binary deployment wizard
...
echo ""
echo "${BOLD}╔══════════════════════════════════════════════╗"
echo "║   Wactorz — Native Deploy Wizard           ║"
echo "╚══════════════════════════════════════════════╝${RESET}"
...
[ -f "./wactorz" ] || die "wactorz binary not found. Run: bash scripts/build-native.sh"
chmod +x ./wactorz
ARCH=$(file ./wactorz | grep -o 'x86-64\|aarch64\|ARM' | head -1 || echo "unknown")
info "Binary: ./wactorz  (arch: ${ARCH})"
...
if [ "${INSTALL_MODE}" = "1" ]; then
    if ! command -v systemctl >/dev/null 2>&1; then
        ...
        SERVICE_FILE="/etc/systemd/system/wactorz.service"
        ...
        sed \
            ...
            systemd/wactorz.service > /tmp/wactorz.service.patched
        sudo cp /tmp/wactorz.service.patched "${SERVICE_FILE}"
        rm /tmp/wactorz.service.patched
...
if [ "${INSTALL_MODE}" = "2" ]; then
    ...
    echo    "╚══════════════════════════════════════════════╝${RESET}"
    echo ""
    echo "${DIM}Press Ctrl-C to stop. For background: nohup ./wactorz --no-cli &${RESET}"
    echo ""
...
# Wactorz — Native Binary Package

Runs **wactorz-server** directly on the host OS.
...
| `API_ADDR` | `0.0.0.0:8080` | wactorz REST API listen address |
| `WS_ADDR` | `0.0.0.0:8081` | wactorz WS bridge listen address |
