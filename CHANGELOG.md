...
- **A voice conversation that gives up now says why in the log.** Turn failures were published to `custom/reachy/events` and nowhere else, so a session that hit its error limit and stopped left the dashboard showing a started conversation, the log showing nothing at all, and a robot that had apparently stopped listening. Failed turns and a failed capture are logged as they happen, with the count against the limit and a note when the session is the one ending.
