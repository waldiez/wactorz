...
def generate_swid(
    ...
    """Readable handle for a Home Assistant device: ``swid:device:<ns>:<slug>-<fp>``.

    A deterministic label (see :mod:`wactorz.core.handles`) — NOT a DID. The
            ...
            output.append(
                {
                    "device_id": device_id,
                    "name": device_name,
                    "swid": generate_swid(device_id, name=device_name),
...
async def get_devices(ws_url: str, token: str) -> list[dict[str, Any]]:
    ...
    readable handle generated by :func:`generate_swid`.
    """
    ws_url = normalize_ha_ws_url(ws_url)
    async with HAWebSocketClient(ws_url, token) as ha:
        devices = await ha.call("config/device_registry/list")
