...
- **A node must run the same version as the server, and is started through a command of its own.** Every node heartbeat names its version; the server now refuses to spawn an agent on, or migrate one to, a node whose version differs from its own, and says which command brings it level (`/deploy <node>`). A node that reports no version at all is a runtime from before the field, and is left to the signing and runtime handling that already covers it. The systemd unit and the unsupervised launcher now start a node with `wactorz-node`, a console script that only releases carrying the node runtime install: on an older release the unit fails with "command not found", where `wactorz --node` used to start that release's *server* because its parser dropped a flag it did not know. `/deploy` checks the command is installed before it starts the unit. `wactorz --node <name>` keeps working for hand-written launchers. `/nodes` now shows each node's version beside its status, and marks one that differs from the server's.

- **`/deploy` proves the node can reach the broker before it reports success.** It used to end at "Runner started", which meant only that the service had been told to start. A node given the wrong broker address, a port a firewall on the broker's host dropped, or a password that was really a comment from `.env.template`, was reported as deployed while it retried every few seconds with the reason in a journal nobody was reading. Now the node opens the broker's port itself before anything is written, the server presents the exact account the node is about to receive and fails if the broker refuses it, and after the node starts the deploy waits for its first heartbeat; if none arrives, the deploy fails and brings back the last lines the node logged. `.env.template` no longer puts a note after an empty deploy setting on the same line, which is what turned one into a password.

- **An edge node runs the Wactorz package, installed on it, rather than a copy of one file.** `/deploy` now installs `wactorz` into the node's venv — at the version main is running, from PyPI, falling back to a wheel built on the main machine and uploaded when that version is not published *or cannot run as a node* — and starts it with `wactorz --node <name>`. That second condition matters while upgrading: a release published before this change answers to its own version number and has no node runtime in it, and an older CLI ignores flags it does not know, so accepting one would have brought the node up as a second server. The deploy asks the install whether it can be a node rather than trusting its version, and reports a failure instead of starting a unit against one that cannot. No extra is needed; everything a node uses is already a core dependency. A node therefore runs the real `DynamicAgent`, compiled from the same code against the same `agent` API and supervised by the same OTP supervisor, instead of a lightweight retelling of that contract that had to be kept in step by hand. Generated code no longer has to be written for one host or the other: `agent.run_in_background`, `agent.send_to_many`, `agent.notify_user` and the time-series queries now work on a node, and `agent.ask_llm` and `agent.set_status` now work on main. `agent.llm` on a node still routes the call through main, so no API key is deployed to an edge device. The node's heartbeat reports `runtime: node`. **A node keeps working as it is until it is deployed again** — main reads an older one's heartbeat as `runtime: runner` — and agent state files are read and written unchanged either way, so no agent loses its memory in the upgrade. `python -m wactorz.remote_runner` and the `--name`/`--broker`/`--port` flags still work, so a unit written against them survives.

- **Commands to an edge node are signed, and a node can refuse one that is not.** A node runs the code a spawn carries, and until now anything that could publish to the broker — any client holding its account, another node included — could send it one. Main now signs every command it sends a node (spawn, desired state, stop, stop all, restart, restart an agent, migrate) with a key derived for that node. `/deploy` writes the key to the node over SSH, and the signature travels in MQTT v5 user properties, so payloads are unchanged. `WACTORZ_NODE_SIGNING` — the `node_signing` option in the add-ons — sets what a node does with a command not signed for it: `warn`, the default, acts on it and reports it, with main saying so in chat, and `enforce` refuses it. **Nothing changes for a node until it is deployed again**: one deployed before this holds no key and checks nothing. A node also refuses a command it has already accepted, or one published before its latest deploy, and main republishes a node's desired state signed once the node reports that it checks. The keys are derived from a secret kept in the state directory as `node_signing.key`; losing it means deploying every node again.

- **Edge nodes reach the broker over TLS.** A node's broker connection carried its account's password and the code of every agent spawned on it in clear text. Wactorz now keeps a private certificate authority in `<WACTORZ_STATE_DIR>/mqtt_tls/` and issues the broker's certificate from it, reissuing it before it expires. The compose broker — from `infra/mosquitto/generated/`, filled under the `python` and `full` profiles or by a run on the host — and the add-ons' embedded broker serve TLS on `8883` beside plain `1883`, and the add-ons write the certificate to `/ssl` for the official Mosquitto add-on. `/deploy` copies the CA to the node and switches it to TLS only after checking, from the node, that the broker answers TLS, so a broker without it leaves the node on plain MQTT; `DEPLOY_<NODE>_BROKER_TLS=on|off` overrides the check, and `DEPLOY_<NODE>_BROKER_TLS_PORT` the port. **A node uses TLS from its next deploy.** `MQTT_TLS_CA` names a CA of your own, or `system` for a certificate from a public CA, and `MQTT_TLS=1` puts the server's own connections on TLS too, on `MQTT_TLS_PORT` (8883): the server creates the CA and certificate at startup when they are missing, writes the broker's copy to `MQTT_BROKER_DIR`, and refuses to start, naming the file, if the CA cannot be loaded. `make mqtt-certs` writes the certificate for the compose broker by hand. A runner told to use TLS that cannot load its CA refuses to start rather than connecting unverified, and a node's heartbeat says whether it uses TLS. `cryptography` is now a direct dependency.

- **Each edge node can have its own broker account.** A node held the server's own broker credentials, so a stolen one could publish anything the server could -- another node's spawn included. With `WACTORZ_NODE_ACCOUNTS=1` (the add-ons' `node_accounts`, on by itself for their embedded broker) every deployed node gets an account named after it, its password derived from the same secret the signing keys come from, so nothing new is stored and a leaked password file says nothing about a signing key. For the brokers Wactorz configures -- compose's and the add-ons' embedded one -- it also generates an access list: a node may publish and read its own `nodes/<name>/...` and the shared agent traffic, and is refused every other node's topics in both directions, `agents/+/commands` and `system/`. The compose broker reloads when a node is added, and restarts itself when an access list or certificate first appears, which mosquitto reads only at startup. For the official Mosquitto add-on, `/share/wactorz/mosquitto-logins.yaml` is written to paste into its configuration -- accounts only, since its own plugin answers before any access list. **A node keeps the shared account until its next `/deploy`**; rotate that account once the last node has moved. `MQTT_TLS_EXPORT` is now `MQTT_BROKER_DIR`, since that folder carries the accounts as well as the certificate. `/deploy` also clears whatever is retained on the node's control topics before starting it: main never retains those, a node acts on what it finds there the moment it subscribes, and an access list can only name the nodes that already exist.

- **Flic buttons as triggers.** A new catalog agent, `flic` (`@catalog spawn flic`), pairs Flic 2 buttons over Bluetooth from chat and publishes every press on a topic of its own, `custom/flic/<serial>/<click|double_click|hold>`, so a button is wired to an action like any other trigger. `@flic pair` says to hold the button down and waits for it; `list`, `rename`, `forget`, `listen`, `stop` and `status` do what they say. Topics are keyed by the serial number printed on the button, so renaming one never breaks what is wired to it; a name keeps the capitals and spaces it was given, and the manifest tells the planner which name is which serial. Each button also keeps a retained `custom/flic/<serial>/state` saying whether it is connected and its battery voltage. The pairing keys are written to a private file in the agent's state folder, and deleting the agent — or a factory reset — deletes them and takes back the retained state. A disconnected Flic sleeps until pressed, so after a restart each button needs one press to wake it, which `listen` and `status` say when a button is not connected. That press is not published unless `@flic late presses on`; a press made while a button was disconnected then arrives when it reconnects, up to a minute late, marked `"late": true`. Needs Python 3.12 or newer, a Bluetooth adapter, and the `flic` extra: `pip install "wactorz[flic]"`. It is not part of `all`.
...
- **Redeploying a node no longer deletes its agents.** A deploy kills the node's old process and starts the new one, and from the server that looked like a crash: the last heartbeats before the kill could already have gone quiet on an agent, and the new process's first heartbeats list no agents until it has reconciled against the desired state. The server counted those as the agent vanishing, removed it from the registry and rewrote the node's desired state without it, so the new node was told to run nothing — a quarter of a second after the deploy's `pkill`. The deploy now tells the server the node is being redeployed: until the node's first heartbeat after the restart, its missing agents are not pruned and its silence does not count as the node going away, and the miss counters are reset when the flag lifts. Agent state on the node was never touched, so an agent lost this way comes back with its memory once spawned again.

- **The compose stacks no longer run the app without an API key.** They declared `WACTORZ_EXPOSED_OK`, which is honest only while the ports stay on `127.0.0.1` — and the file's own comment said to drop that prefix to expose them, which then served an open API, agent spawning included, with nothing refusing it. Every container on `wactorz-net` could reach it regardless, Home Assistant under the `full` profile among them. Now a one-shot `api-key` service generates a key on first start when `.env` sets none, keeps it in a volume, and the app and Prometheus both read it; `API_KEY` in `.env` still wins. Read the generated key with `docker compose exec wactorz-python cat /run/wactorz/api_key`, or follow the one-time sign-in link in the app's log. The server reads a key from the file `API_KEY_FILE` names when `API_KEY` is unset, which also suits Docker and Kubernetes secrets. **Plain MQTT on `1883` is now published to the compose host only**, since it carries the broker password in the clear; remote nodes use TLS on `8883`, and a node still on plain MQTT needs `MQTT_EXTERNAL_BIND=0.0.0.0` in `.env` until it is deployed again. `.env.template` no longer lists `MONITOR_EXTERNAL_PORT`, which nothing read: the monitor is published on `MONITOR_PORT`. **Upgrade with `docker compose --profile python up -d --build`**: an image built before this ignores the generated key, and the app then refuses to start rather than run without one.

- **The Home Assistant add-ons no longer trust every other add-on.** The ingress bypass was accepted from the whole Supervisor network, `172.30.32.0/23`, where every add-on lives and any of them can set the ingress header. It is now accepted from the Supervisor's own address, `172.30.32.2`, the one Home Assistant tells add-ons to trust; `WACTORZ_INGRESS_PEERS` still overrides it. The add-ons also no longer run without an API key: with `api_key` blank, one is generated on first start, kept in `/data` and never shown, so another add-on can no longer reach ports `8000` and `8888` unguarded and spawn agents, and the add-ons stop declaring `WACTORZ_EXPOSED_OK`. The panel is unaffected, since ingress never consults the key. **If you published port `8000` or `8888` without an `api_key`**, set one; until you do, those ports answer only with a sign-in page or 401. Secret options are now masked in the add-on configuration screen.

- **`X-Forwarded-*` headers are believed only from a proxy you list.** The host check that stops DNS rebinding read `X-Forwarded-Host` from any caller, and a rebound page can send that header on its own same-origin requests without a preflight — so on a loopback install with no key, a web page could name `localhost` and drive the API, spawning agents included. `WACTORZ_TRUSTED_PROXIES` (addresses or CIDRs, empty by default) now names the proxies whose forwarded headers count; from anyone else they are ignored, and the first ignored one from each address is logged. The host check, the origin check, the session cookie's `Secure` flag and the sign-in throttle all read through that one rule, so behind a listed TLS-terminating proxy the cookie is now marked `Secure` and one guesser no longer locks every user out. **A setup behind a reverse proxy that relies on `X-Forwarded-Host`** needs `WACTORZ_TRUSTED_PROXIES` set to the proxy's address, or the proxy must pass the original `Host` through. Home Assistant ingress is unaffected.

- **One value an agent cannot store no longer costs it everything else it remembers.** A node keeps an agent's state as JSON, and the save streamed straight into the opened file — which truncates it first and then stops at the first value that will not serialise. A single `agent.persist('seen_at', datetime.now())` left invalid JSON where a good file had been, and the next start read that as unreadable and began empty, so the agent lost its counters, its calibration and its history at once. The state is now encoded before anything is written and the file replaced in one step, so an interrupted save leaves the previous contents; a value that cannot be written as JSON is dropped on its own and named in the log, as it already was when an agent migrates.

- **A program a node repaired stays repaired.** An agent whose code fails can be fixed by the LLM in place, and every route that rebuilt it used to hand back the program that failed — so a repaired agent was broken again by the next restart, by `/nodes restart-agent`, by being migrated home, or by the node rebooting, and was repaired over again each time. The node now keeps the running program where the first three read it, and main's spawn registry is brought up to date for the fourth. That last step is deliberately indirect: the node publishes a *notice* that a program changed, carrying no code, and main then asks for it on a signed topic quoting a token it minted and spends on use. Main acts on code a node sent only in answer to a question main asked — the same rule a migration follows — because code a node could volunteer would be filed for any agent it hosts and would then run wherever that agent went next. A forged notice costs one question and one answer.

- **Generated code that opens its own broker connection now dials the real broker.** `agent._mqtt_broker` and `agent._mqtt_port` — which the agent API exposes so a program can create an `aiomqtt` client of its own — were copied when the agent was built, and every agent is told its broker just *after* that: by the parent it was spawned from, or by the node running it. So they read `localhost:1883` for every agent, whatever the install actually used. Unnoticed wherever the broker really is local; wrong for the Home Assistant add-ons, a broker on another machine, and every agent on a node. They now read through to the agent itself, so there is no copy to go stale.

- **`agent.window()` no longer opens a broker connection every time it is called.** Generated code calls it from a process loop as readily as from `setup()`, and each call built a new window on a new connection — so the buffer being read was always the empty one just created, and the connections accumulated. A window is now created once per topic per agent, and starting one twice runs a single listener.

- **A contract an agent declares no longer discards what an earlier call declared.** An agent that declared its outputs and its inputs in two `declare_contract` calls — which models write often — kept only the second. Each call now adds to what is already known. On a node, where the spawn config's `publishes`, `subscribes`, schemas and `capabilities` now reach the agent, they are folded in as well and carried in its manifest, so the planner sees what an agent is for before it has published anything.
...
- **Old data is now deleted on a schedule, so a long-running install stops filling its disk — which means chat history older than a year is deleted by default.** Nothing was ever pruned before: the chat history, the sensor and Home Assistant history and attached files grew for the life of the install, and a message the MQTT broker never accepted was kept and retried on every start, for ever. The server now deletes chat turns and time-series rows older than 365 days, and any attached file no remaining message refers to — or a day after it was uploaded, if it was never sent. A message still undelivered after 7 days is removed from the outbox, so it is no longer retried after a restart, and the log names its topic. Each window is a setting, `WACTORZ_RETENTION_CHAT_DAYS`, `WACTORZ_RETENTION_TIMESERIES_DAYS` and `WACTORZ_RETENTION_OUTBOX_DAYS`, where 0 keeps that store for ever — **set `WACTORZ_RETENTION_CHAT_DAYS=0` before upgrading to keep all of your chat history.** A deleted file frees its space at once; the two database files stop growing but do not shrink, because SQLite reuses the space it frees rather than returning it to the disk. A message whose file is gone still shows what was attached. The time-series collector agent keeps pruning by its own window, and with both in force the shorter one holds.

- **A node's heartbeat now says which version of Wactorz it runs, and what kind of process is answering.** Every heartbeat on `nodes/<name>/heartbeat` carries `version` and `runtime`, and the node list on the dashboard API and in `/nodes` shows both. A node deployed before this change sends neither; the server records it as the single-file runner at an unknown version rather than treating the silence as an error. This is the first step towards nodes that run the Wactorz package instead of a copied script: the server can now tell the two apart, and warn about a node that is behind. The contract between server and node — the topics, the heartbeat and spawn fields, and the state file a node keeps — is now pinned by a test, so a runtime that replaces the runner is held to the same one.

- **A deployed node now runs under systemd, so it comes back after a reboot.** The runner was launched with `nohup`, which meant a power cut left the node gone until someone redeployed it by hand, and a crash left it gone until someone noticed. A deploy now installs a `wactorz-node` unit, choosing the least privilege the machine actually offers: root if the deploy user is root, otherwise a user unit when lingering is enabled, otherwise passwordless sudo. When a machine offers none of those the old `nohup` launch still happens, but the deploy result now says `nohup — unsupervised` rather than reporting the same success as a supervised install — the two used to be indistinguishable until the power blinked. Redeploying an existing node stops its old runner first, and clears any unit left at a location an earlier deploy chose, so a node whose privileges changed does not end up starting two runners after a reboot. `~/wactorz/.env` now also carries the node's name, broker and port, so the unit needs no per-node text. The deploy also asks the node where the deploy user's home directory is rather than assuming `/home/<user>`, which is wrong for root and for homes not laid out under `/home` — uploads landed in one place while the virtualenv was built in another, so deploying as root could never work.
...
- **Moving an agent between two remote machines now goes through the server, and the agent is not removed from where it started until the new machine confirms it is running.** Previously the source machine handed the agent directly to the target and deleted its own copy immediately: if anything was lost in between, the agent was gone from both. It also meant one machine could tell another to start code, which is a capability no edge device should have. The server now asks the source to hand the agent back, places it on the target itself, waits to be told it started, and only then tells the source to delete its copy. If the target never confirms, the agent is put back where it was — it was stopped there, never deleted, so nothing has to be reconstructed. Migrating an agent back to the server works the same way, and a migration that cannot be completed locally no longer deletes the remote copy.
...
- **Deleting an agent now asks first, and names the agent it is about to remove.** Delete is the one thing you can do to a card that has no way back, and it happened on a single click with nothing in between — the button beside it, Stop, is undone by pressing Start again. A confirmation now opens naming the agent; cancelling it, pressing Escape or clicking outside it sends nothing. It is drawn as part of the dashboard rather than by the browser, so it matches the rest of the page and does not freeze everything else on it while it waits for an answer.

### Fixed

- **A message addressed to an agent name that does not exist no longer leaves the dashboard waiting.** Typing `@weather` where the agent is called `weather-agent` left the thinking dots up and the send button disabled for a minute, and the answer — "Agent @weather not found." — arrived only as a notification, in no conversation at all. The dashboard treats a mention it does not recognise as ordinary text and keeps you in the conversation you are in, while the server answered as the agent that had been named: a reply from an agent that does not exist, which the dashboard has no conversation to show it in and cannot match to the turn it is waiting on. Such a turn now belongs to the conversation it was sent from, so the refusal arrives there as a reply and ends the turn at once. Both halves of the exchange are kept in that conversation's history too, where before they were filed under the name that was not found and never shown again. A message to an agent that does exist, including one running on a remote machine, is unchanged.

- **One MQTT message the server cannot send no longer stops every message behind it.** The server sends its messages one at a time, in order, and sent a failed one again before anything else — for ever, if it could never succeed. One such message held up everything queued after it: spawns, stops, the acknowledgements that finish a migration. A message it had stored came back after a restart and held them up again. A topic containing `+` or `#`, or an empty one, was enough, and agents choose their own topics. Such a topic is now refused when it is published, with a warning naming it; a message the MQTT library refuses is dropped instead of retried; and a message that fails five times in a row on a working connection is dropped as one the broker will not take, with a warning naming its topic. An agent can no longer be given a name with `+` or `#` in it, or an empty one, since every topic it uses would carry it; one saved under such a name before is not brought back after a restart, and the log says why.

- **A conversation with the main agent from outside the dashboard is now kept in the chat history as it was actually had.** From Telegram, Discord, WhatsApp, the REST interface or the command line, main kept only the turns its model answered, and kept them wrong: the message was stored with an internal block listing the running agents in front of it, and the reply was the model's raw output — on a social channel, even a reply that was withheld and replaced before it was sent. Answers main gives without asking its model — controlling a device, a Home Assistant question, a pipeline, the planner, a refusal — were not kept at all. Every one of these is now kept once, as the words typed and the reply that was sent. A voice conversation through Reachy, which Reachy already shows in the chat, is no longer stored a second time under main. Turns held on the dashboard are unchanged, and rows already stored are left as they are.

- **A conversation with an LLM agent is now kept in the chat history whichever model provider it uses.** An agent wrote its conversations to the history the dashboard shows only when its model returned the answer all at once. When the answer streamed in — the usual case — the agent still kept the conversation in its own memory but never wrote it to that history, so anything that did not come through the dashboard itself, such as a message sent through the API, was missing from the feed after a reload. A streamed answer is now recorded the same way, once it has finished; an answer interrupted part-way is not, just as before for the non-streamed kind. Conversations held on the dashboard are still recorded once.

- **A message sent from the dashboard to the Home Assistant, Gmail or Calendar agent no longer appears twice.** The dashboard stores every conversation turn it carries, and those three agents stored their own turns as well, so each exchange with them was saved twice and shown twice in both the chat and the feed. The second copy also skipped the step that hides passwords and tokens typed into a message, which the dashboard's copy applies. The agents now leave a turn alone when the dashboard has already stored it, and still store it themselves when it arrives any other way — through the API, where theirs is the only record — with secrets hidden the same way. Turns already saved twice are left as they are.
...
- **Spawning, stopping and chatting no longer stall every agent in the process for a moment each.** A message that must not be lost is written to a small database before it is sent, and that write opened, committed and closed a connection of its own — twice per message, once to record it and once to tick it off. On the SD card a Raspberry Pi runs from, that pair of writes takes about sixteen milliseconds, and for every one of them the whole system stops: not just the agent that sent the message, but every other agent, and the keepalive that tells the broker this machine is still alive. It is paid on exactly the operations a person is waiting for, because commands to nodes and tasks to agents are the traffic that must not be lost — sensor readings and heartbeats are allowed to be, and never touch it. The connection is now opened once and kept, which is also what makes it possible to tell the database it need not force every write to the card immediately. Both halves are needed: keeping the connection alone saves almost nothing, and the setting alone is *slower* than before. Together they take those sixteen milliseconds to under a tenth of one. A connection that goes bad is dropped and reopened on the next write, so a passing fault stays passing.
  ...
  Not forcing every write to the card immediately means they accumulate, and folding them back in is itself a pause — a longer one, about sixty milliseconds, which the database would otherwise take on whichever message happened to cross its threshold. That would have replaced a stall on every message with a worse one now and then, so it now happens on a background thread while nothing is being sent, and the point at which the database would do it itself is raised to a level the scheduled one normally reaches first. Raised rather than switched off: if the background work ever stopped, switching it off would let the file grow until the disk filled, which is a worse failure than the pause it avoids.

- **An agent removed from a node no longer comes back the next time that node reboots.** The message that tells a node to run an agent was retained on the broker, which means it is handed to the node again every time it reconnects — long after the decision that produced it stopped being true. Anything that withdrew the agent afterwards, a migration rolled back or a delete, was a one-time instruction competing with a standing one, so the agent reappeared on the next reboot and ran alongside whatever had replaced it, two copies under one name drifting apart from the same starting state. Withdrawing that message was not possible either: it is addressed to the node rather than to the agent, so clearing it would have discarded whatever instruction was waiting in that slot, which may have been for something else entirely. Reaching a node that was switched off is the job of the retained description of what a node should be running — which is kept up to date, and which a rollback now empties — so the instruction itself no longer needs to outlive its delivery. A node that is merely offline for a while still receives it when it returns, along with anything that came after it.

- **The retry policy added for busy and unreachable providers now actually engages.** It was written to wait out rate limits, dropped connections and timeouts, and in practice almost none of them reached it. A refused or dropped connection arrives from the hosted SDKs as a class of their own that is neither a timeout nor an operating-system error and carries no HTTP status, so it was classified as a request that would fail identically however many times it was sent; the socket error underneath is now found by following what the exception was raised from, including the link that survives when a library deliberately discards the one before it. Matching on that rather than on the HTTP library's own classes also keeps this working when an SDK ships a private copy of that library, which is not the one this process imports and would not have matched. Two providers hid their failures in ways of their own. NIM answered every failed tool call with the same advice about the model not supporting tools, which turned a rate limit into a sentence about something else and dropped the status the policy reads — it is now passed through untouched when another attempt could help, and still explained when it could not. Gemini's streaming never raised at all: a failure was turned into text and delivered as though the answer had finished, so nothing was ever asked whether to try again, and a rate limit reached the user as an error message. It now raises while nothing has been read yet, which is the only point where a second attempt is safe; once part of an answer has been delivered the failure is still reported alongside it, because retrying then would repeat text the reader has already seen. A stream that goes silent before saying anything is treated the same way.

- **A migrated agent now keeps what it learns, and a migration that fails leaves it in exactly one place.** Three things went wrong once an agent had been moved. The state snapshot handed over during a migration is applied by taking the agent's state file away and writing the snapshot in its place — right on arrival, because the source held the authoritative copy at that moment, and wrong every time afterwards, because the agent has been living and accumulating on the target ever since; that snapshot was being kept in both of the records a node reads back, so a reboot reconciled the agent, found it still attached, and rolled the agent back to the moment it arrived, losing more the longer it had been there. Those records now describe what the agent *is*, and the snapshot rides only on the spawn message it belongs to, which the runner already clears once it has been consumed — upgrading is enough to fix an agent already in this state, since a snapshot left behind by an older version is dropped as the entry is read. Moving an agent from this machine out to a node also deleted the local copy before the node had it: it snapshotted the state, stopped the agent, wiped everything it had persisted, and only then sent it out, so a spawn that failed on the node for a compile error or a missing package left the agent in neither place. That copy is now kept until the node confirms the agent is running, and nothing is written to the spawn registry until then either, so a restart mid-migration brings the agent back rather than recording it somewhere it never arrived. Finally, a migration that timed out told the target to stop the agent but left it in that node's standing orders, so the node's next reboot started a second copy beside the one the rollback had just restored; the rollback now withdraws it there too.

- **A busy or unresponsive model no longer fails the request outright, or ties up an agent until the provider library gives up.** A rate limit answered the user with an error, even though waiting a moment and asking again is what the provider was asking for, and a request that hung held the calling agent until the provider library gave up on its own — a default that nothing here had chosen. Transient failures — rate limits, server errors, timeouts and dropped connections — are now retried with exponential backoff, and an attempt that runs longer than `LLM_TIMEOUT_S` is abandoned and retried. Errors that another attempt cannot fix, such as a rejected key or a malformed request, still fail immediately rather than costing three times the latency to say the same thing. A streamed reply is retried only while nothing has reached the reader, since a second attempt part-way through would repeat text they have already seen; the timeout there bounds the wait for the first chunk, so a connection the service never answers is let go while a long answer still runs to the end. When a provider says how long to wait, that number is used instead of our own backoff — it knows when its limit resets and we are guessing — unless it asks for longer than a minute, at which point failing is kinder than holding an agent. A request that runs out of attempts now fails with a message saying the provider was unavailable or rate-limiting us, so it no longer reads like the request was malformed. The spend cap is checked once per call rather than once per attempt: retries that were rate-limited cost nothing, and counting them would let a rate-limit storm trip a cap it never spent against. This sits in the same place as the spend cap, so it covers every provider — hosted and local alike — and a new one inherits it without carrying any retry code of its own. `LLM_MAX_RETRIES` and `LLM_TIMEOUT_S` set the policy; either can be set to 0 to turn its half off.

- **An agent whose code the model repaired at run time now actually runs the repaired code.** When `process()` kept crashing, the model was asked for a fix, the fix compiled and was saved for restarts, and the log said the agent was resuming with the patched version, but the loop went on calling the function it had before the repair, so the same crash continued until the Supervisor restarted the agent and only then picked up the fix. A repair now stops the old program (its process loop, its subscription listeners, anything it left open), then starts the repaired one from its own `setup()`, so module state the new code expects is built. The same stale-function mistake in the `setup()` repair path is fixed too. A crash gets a bounded number of repair rounds before the Supervisor takes over, the follow-up prompt tells the model what its previous fix still failed with, and every repair prompt carries the agent's description so the fix serves what the agent is for.

- **A node that has stopped responding is no longer reported as online.** Nodes send a heartbeat every few seconds, and the server records the moment each one arrives. Since heartbeats were being sent in a mode that asks the broker to hold undelivered messages, a server that had been disconnected received a backlog of old heartbeats on reconnect and treated each as proof the node was alive right then — so a machine that had been dead for hours appeared online, and the server would agree to move an agent onto it. Heartbeats and other routine status are now sent without that guarantee, so nothing is held and replayed; commands and results, where late delivery is still better than none, keep it.

- **Commands sent to an agent on another machine are no longer silently lost.** Stopping, deleting or restarting an agent that runs on a remote node is delivered over the broker, and both ends were asking for the cheapest, least reliable mode: a command issued while the agent was briefly reconnecting simply vanished, leaving the dashboard reporting it stopped while it carried on. The same applied to clearing an agent's stored setup, where a lost message could bring a deleted agent back on the node's next check. Both now use at-least-once delivery.

- **Chat and error messages from an agent survive a dashboard reconnect.** They were sent in the mode used for routine metrics, so anything an agent said while the dashboard's connection was re-establishing was gone — the agent had spoken and nobody saw it.

- **An agent that reacts to Home Assistant triggers no longer misses one while reconnecting.** Its listener kept no session with the broker, so a request to switch a light or a device that arrived during a reconnect was dropped, and nothing recorded that it had been.

- **A node no longer runs out of memory when the broker is unreachable.** Everything a node publishes — heartbeats, logs, agent results — waits in memory until the broker accepts it, and that queue had no limit: a broker that went away while agents kept working grew it until the machine, often a Raspberry Pi, ran out. The queue is now capped. When it fills, the oldest telemetry is discarded first, because the next heartbeat replaces it anyway and the freshest reading is the useful one; results, errors and the replies that decide where an agent lives are kept in preference to it. Publishing never waits for room, since making it wait would stall the agent that called it every time the broker was slow. Discards are counted and reported.

- **Messages sent to the server or to a node while it was reconnecting are no longer lost.** The server's listeners and each node's control channel asked the broker for messages at "at most once" and told it to forget them the moment the connection dropped, so anything published during a restart or a network blip was discarded rather than delivered. Only messages the broker had been asked to retain survived, which covered agent spawning and little else -- a stop, a restart, a migration or a delegated task issued at the wrong moment simply never arrived. Those connections now keep their subscriptions registered with the broker and ask for at-least-once delivery, so the broker holds what it could not deliver and hands it over on reconnect. Every kept session names how long it may be kept — a day for the server and its nodes — so an installation that is retired, or a node that never comes back, stops costing the broker memory instead of being remembered indefinitely. A named agent's subscriptions are covered too, and are held for an hour after it goes away — long enough to cover a restart or a reboot, short enough that an agent which is deleted, or a node that never comes back, does not leave the broker holding state for ever. An agent with no name of its own is not covered, because it is given a new identity each time it starts and could never reclaim what was held for the old one.

- **Two Wactorz servers sharing one MQTT broker no longer disconnect each other in a loop.** The publisher identified itself to the broker as `wactorz-publisher`, a fixed name with nothing to say which install it was. MQTT allows one connection per name, so a second server — a development instance pointed at the same broker was enough — took the name, the first reconnected and took it back, and neither stayed connected. Every long-lived connection now carries a name built from a per-install identifier written to the state directory on first run, so installs no longer collide and a reconnecting connection is recognised as the same one rather than as a stranger. The six long-lived listeners inside the server are named separately too; they previously connected anonymously, which worked only because the broker assigned each a random name. On a remote node the runner's two connections are now named after the node rather than randomly, which makes a duplicate runner visible instead of silent: two runners started under one node name now disconnect each other in a loop rather than both staying connected and both acting on the same commands. A node deployed twice by accident announces itself instead of quietly doubling up.
...
- **A pipeline plan whose generated code contains a Python backslash escape no longer fails to parse.** The planner now repairs backslashes that are not valid JSON escapes before parsing, instead of falling back to a direct answer.

- **The main assistant now knows it speaks as Wactorz** and answers "who are you" and "what can you do" in user terms rather than reciting its internal manual.
...
- **An agent that ends by itself now leaves the dashboard, instead of its card flickering in and out.** Agents that finish on their own — a planner that has answered, a one-off action that has been carried out, a schedule set to fire once — took themselves down but left the server still holding a record of them. The page then rebuilt its grid from two sources that disagreed: the list of running agents no longer named the agent, so every refresh dropped the card, while the running update the server pushes still carried it, so it reappeared. An agent that ends now withdraws the announcement it published when it started. That withdrawal is the one message every ending already sends, including from an agent running on another machine, so the dashboard can read it wherever the agent lived, and it takes it as the agent being gone. An agent that was retired after crashing past its restart budget is deliberately left on screen — that card is the thing the notification about it is asking you to look at.

- **The application CPU meter no longer reads above 100%.** It reported the figure `top` would give, which is a share of a single core: a process keeping two and a half cores busy read 263%, while the bar beside it was drawn on a scale that stops at 100, so the bar sat pegged at full and disagreed with its own label. The figure is now a share of the whole machine, so the two agree and the meter means what it appears to mean. On a machine where the number of cores cannot be determined the reading is left as it was.
...
- **A guarded install has a dashboard again: sign in with the key, browse with a session.** Setting `API_KEY` protected the API and locked the browser out with it, because a page cannot attach a header to its own navigation. There is now a sign-in page: enter the key once and the browser holds a session cookie instead. **The cookie carries a random session id and never the key** — so it can be revoked on its own, and signing out does not disturb the scripts, probes and integrations that use the key directly. **On startup, with a key set, the terminal prints a one-time sign-in link** that skips typing the key; it works once, expires in fifteen minutes, and is printed rather than logged, so it never reaches the log file or the dashboard's log view. Sign out from this browser, or from everywhere at once. **Nothing changes for an install with no key** — that is still the default, still open, still protected by listening on `127.0.0.1` rather than by a credential. Home Assistant add-on users see no sign-in at all: the panel is already behind Home Assistant's own login.

- **The dashboard reopens on the agent you were last talking to.** The chat always started on `main` (or the first agent alphabetically) however many times you had picked someone else, so every reload put you back at the beginning. The agent you pick is remembered, and so is one you reach by `@mention`, since the conversation follows the message there. What is not remembered is a move the dashboard made *for* you, because the agent went away. The remembered name is checked against the agents that actually exist before it is used, so one that has since been deleted quietly falls back to the usual default. An agent that is merely stopped is still reopened, as it would be without a reload.
...
- **Leaving `TTS_VOICE` empty now gives you the default voice, as `.env.template` says it should.** The template ships the setting blank under the line "Leave TTS_VOICE empty for the default", but the code asked for it with `os.getenv("TTS_VOICE", default)` — and a default argument applies only when a name is *absent*, not when it is present and empty. `.env` made it present and empty, so `""` travelled all the way to the speech synthesiser, which refused it: every attempt to speak failed with `Invalid voice ''` and nothing else to go on. **Anyone who copied the template and left that line alone had no speech at all**, while anyone without a `.env` was unaffected, which is why it survived. Whitespace is treated the same way now, since a `.env` line keeps trailing spaces more often than anyone intends.

- **Agents that cannot hold a conversation now say so, instead of replying with a dump of their own state.** Asking an actuator "how warm is the nursery?" returned `{'automation_id': ..., 'actuations_count': 1, ...}` — and asking it anything else returned the identical text, because the question was never read. The chat surface builds a reply by looking for a `reply`, `message`, `text`, `content` or `result` field and falling back to printing the raw payload; the Home Assistant actuator, the entity-map agent and the state bridge all answered with dicts that had none of them, as did a dynamic agent's three failure paths. Each now answers in a sentence that says what it is for and, where it applies, that it cannot answer questions — an actuator waits on a topic and calls a service, and telling you that is more use than telling you its actuation count. **Nothing changes for callers**: the structured fields are still there for the agents that delegate to these and read them, with the sentence added alongside. The two ways a reply reaches the chat also read it the same way now — an agent answering in this process was read `reply` first while one answering from a runner node was read `result` first, so moving an agent onto a node could have changed what it appeared to say, with nothing to point at. Both take `result` first, which is the field the prompts tell an agent to fill.
...
- **A port already in use is reported as one line, not a traceback.** Startup checks the port first, but that check binds and releases, so something else can still take it in the moment before the real bind — and when it did, the failure surfaced as an aiohttp traceback with the actual cause several frames down. The bind that matters now reports for itself, names the port, and exits; the half-built server is torn down on the way out rather than left holding its listener. The earlier check remains as what it always was — a way to fail sooner and more clearly in the common case, not a guarantee.

- **An unrecognised URL can no longer inflate the metrics.** `/metrics` labels each request with the route it matched, but a request that matched nothing was labelled with the path as typed. Anything that walks a server looking for URLs — a scanner, a stray crawler, a broken link followed repeatedly — therefore added a permanent new series per distinct path, growing the metrics endpoint without bound and taking the scrape and its storage with it. Unmatched requests now share one label and are still counted.
