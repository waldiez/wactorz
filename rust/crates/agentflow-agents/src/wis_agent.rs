...
fn note_index(s: &str) -> Option<usize> {
    match s.to_uppercase().replace("♭", "B").replace("♯", "#").as_str() {
        "C" | "B#"  => Some(0),
        "C#" | "DB" => Some(1),
        "D"          => Some(2),
        "D#" | "EB" => Some(3),
...
fn parse_root(token: &str) -> Option<(usize, &str)> {
    ...
    if !token.is_empty() {
        if let Some(idx) = note_index(&token[..1]) {
...
fn build_scale(root: usize, mode: &str) -> Option<(&'static str, Vec<usize>)> {
    ...
    let (name, intervals): (&'static str, &[usize]) = match m.trim() {
        ...
        "minor" | "natural minor" | "aeolian" | "min"      => ("Natural Minor (Aeolian)", &[0,2,3,5,7,8,10]),
...
impl WisAgent {
    ...
    fn cmd_add(&self, parts: &[&str]) -> String {
        ...
        if parts.len() < 2 {
            return "Usage: `add <song|album|artist> \"<title>\" [artist] [rating] [genre]`\n\n\
                    ...
                    • `add artist Bach 10 baroque`".to_string();
        }
        ...
        let (artist, raw_rating, raw_genre): (Option<String>, Option<&str>, Option<&str>) =
            ...
            _ => {
                ...
                let is_rating = first.map(|s: &str| s.parse::<f64>().is_ok()).unwrap_or(false);
                if is_rating {
                    (None, first, rem.get(1).copied())
                } else {
                    (first.map(|s| s.to_string()), rem.get(1).copied(), rem.get(2).copied())
                }
            }
        };
        ...
        let icon = match entry_type.as_str() { "song" => "🎵", "album" => "💿", _ => "🎤" };
    ...
    fn cmd_stats(&self, filter: Option<&str>) -> String {
        ...
        let entries: Vec<&MusicEntry> = match filter {
            ...
            Some(g) => {
                ...
                log.iter().filter(|e| e.genre.as_deref().map(|eg| eg.eq_ignore_ascii_case(g)).unwrap_or(false)).collect()
            }
        ...
        let avg = if total_rated > 0 { total_rating / total_rated as f64 } else { 0.0 };

        let mut rows = Vec::new();
        for t in &["song", "album", "artist"] {
            if let Some(&(count, rs, rc)) = by_type.get(*t) {
                let avg_t = if rc > 0 { format!("avg ⭐ {:.1}", rs / rc as f64) } else { "unrated".to_string() };
                let icon = match *t { "song" => "🎵", "album" => "💿", _ => "🎤" };
                rows.push(format!("  {icon} **{}s**: {count} — {avg_t}", Self::capitalize(t)));
            }
        }
        ...
        format!("{header}\n\n{}\n\n**Total**: {total} entries{avg_line}", rows.join("\n"))
    }
    ...
    fn cmd_top(&self, parts: &[&str]) -> String {
        let n: usize = parts.first()
            ...
            .unwrap_or(5)
            .min(20);
        let type_offset = if parts.first().and_then(|s| s.parse::<usize>().ok()).is_some() { 1 } else { 0 };
        let filter = parts.get(type_offset).copied();

        let log = self.log.lock().unwrap();
        let mut rated: Vec<&MusicEntry> = log.iter()
        ...
        let type_label = filter.map(|t| format!(" — {}s", Self::capitalize(t))).unwrap_or_default();
        let rows: Vec<String> = rated.iter().take(n).enumerate().map(|(i, e)| {
            let icon    = match e.entry_type.as_str() { "song" => "🎵", "album" => "💿", _ => "🎤" };
            let artist  = e.artist.as_deref().map(|a| format!(" — _{a}_")).unwrap_or_default();
            let genre   = e.genre.as_deref().map(|g| format!(" `{g}`")).unwrap_or_default();
            format!("  {}. {icon} **{}**{artist} ⭐ {:.1}{genre}", i + 1, e.title, e.rating.unwrap_or(0.0))
        }).collect();
    ...
    fn cmd_theory(&self, parts: &[&str]) -> String {
        match parts.first().copied().unwrap_or("") {
            ...
            "bpm" => {
                ...
                format!(
                     ...
                     **Delay times**\n\
                     ...
                     Dot 1/8 : {:.1} ms\n\
                     1/16 : {:.1} ms",
                    bar_4_4 * 2.0, half_ms, beat_ms, eighth_ms, sixteenth,
                    delay_8th, delay_dot8, delay_16,
                )
            }
            ...
            sub => format!("Unknown theory sub-command `{sub}`. Use: `chord`, `scale`, `bpm`, `interval`."),
        }
    }
