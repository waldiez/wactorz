...
- **There is an end-to-end test suite.** The release checklist — a real broker, the app as a process, a second process standing in for an edge node, and a browser — was a manual walkthrough somebody did before a tag, which meant it was done differently each time and skipped when time was short. `make e2e` now runs it: the regression core covers the seams a unit test structurally cannot reach (browser to socket, agent to broker, main to node, container lifecycle), and `make e2e-release` adds the pre-tag walkthrough on top. Answers come from a provider that does not call out, so a run costs nothing and produces the same result twice; the same scenarios can be recorded against a real model with `make e2e-demo`, and a recording is therefore a recording of a passing test. **This is not a required check** — real processes and a browser give it more ways to be non-deterministic than the unit suite has, and a flaky required check is worse than none. It runs on demand and before a tag. `make e2e-setup` installs the browser once; see `e2e/README.md`.
