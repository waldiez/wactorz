...
import { CardBabylonTheme } from "./themes/CardBabylonTheme";
import { GraveTheme } from "./themes/GraveTheme";
import { CardDashboard } from "../ui/CardDashboard";
import { SocialDashboard } from "../ui/SocialDashboard";

export type ThemeName = "graph" | "galaxy" | "cards" | "cards-3d" | "grave" | "social";
...
class NullTheme extends ThemeBase {
  ...
  setup(): void {
    ...
    const light = new HemisphericLight("null-ambient", new Vector3(0, 1, 0), this.scene);
    light.intensity = 0.2;
  }
...
export class SceneManager {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.scene.clearColor.set(0.02, 0.03, 0.10, 1); // deep indigo
  ...
  setTheme(name: ThemeName): void {
    ...
    const leavingHTML = ["cards", "social"].includes(this.activeTheme.name);
    const enteringHTML = name === "cards" || name === "social";
    const entering3D = name === "cards-3d";

    // Restore camera when leaving any HTML/cards mode back to a 3D view
    if (leavingHTML && !enteringHTML && !entering3D) {
    ...
    if (name === "cards") {
      ...
      if (entering3D) {
        ...
        this.activeTheme = new CardBabylonTheme(this.scene);
      } else if (name === "galaxy") {
        this.activeTheme = new GalaxyTheme(this.scene);
      } else if (name === "grave") {
        this.activeTheme = new GraveTheme(this.scene);
      } else {
  ...
  onAgentSelected(agentId: string): void {
    if (this.cardDashboard || this.socialDashboard) return; // HTML overlay — no camera to pan
    ...
    Animation.CreateAndStartAnimation(
      "camPan", this.camera, "target",
      30, 30,
      this.camera.target.clone(), node.position.clone(),
