...
- **The dashboard reopens on the agent you were last talking to.** The chat always started on `main` (or the first agent alphabetically) however many times you had picked someone else, so every reload put you back at the beginning. The agent you pick is remembered, and only that: a conversation the dashboard moved *for* you — because the agent went away — is not remembered as a choice. The remembered name is checked against the agents that actually exist before it is used, so one that has since been deleted quietly falls back to the usual default. An agent that is merely stopped is still reopened, as it would be without a reload.
