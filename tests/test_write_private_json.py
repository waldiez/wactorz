"""A file written by `write_private_json` is never readable by anyone but its owner.

It holds secrets: the Google MCP integration's OAuth tokens, and the pairing
keys of the Flic agent's buttons. A moment spent at the umask's permissions is a
moment in which any account on the machine can copy them — and a refresh token
or a pairing key is good until it is revoked, so copying it once is enough.
Narrowing the mode after the write leaves exactly that window, and leaves the
file readable for good on any platform or filesystem where the narrowing fails.
...
def test_the_parent_directory_is_created(tmp_path: Path) -> None:
    ...
    write_private_json(target, {"tokens": {}})
...
def test_rewriting_keeps_the_permissions(tmp_path: Path) -> None:
    ...
    target = tmp_path / "token.json"
    write_private_json(target, {"tokens": {"access_token": "first"}})

    write_private_json(target, {"tokens": {"access_token": "second"}})
...
def test_a_failed_write_leaves_the_previous_file_whole(tmp_path: Path) -> None:
    ...
    target = tmp_path / "token.json"
    write_private_json(target, {"tokens": {"access_token": "keep"}})

    class Unserialisable:
        pass

    with pytest.raises(TypeError):
        write_private_json(target, {"tokens": Unserialisable()})  # pyright: ignore[reportArgumentType]
