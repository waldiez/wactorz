 ...
 *
 * Formerly a Babylon.js scene manager; the 3D themes were removed (only the
 * cards view remains), so this is now a plain data/overlay coordinator with no
 * 3D engine. The class name and public API are kept to avoid churn in main.ts.
 */
...
export class SceneManager {
  ...
  // The canvas param is kept for call-site compatibility but is no longer used
  // (there is no 3D engine anymore).
  constructor(_canvas?: HTMLCanvasElement) {}

  // ── Theme (only "cards" remains) ──────────────────────────────────────────

  setTheme(_name: ThemeName = "cards"): void {
    if (this.cardDashboard) return;
  ...
  onHeartbeat(payload: HeartbeatPayload): void {
    ...
    if (agent) {
      ...
      this.cardDashboard?.onHeartbeat(
        payload.agentId, payload.timestampMs, payload.cpu, payload.memory_mb,
      );
    } else {
