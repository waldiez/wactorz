...
class SubscriptionHub:
    ...
    def _stop_worker(self, binding: _Binding) -> None:
        ...
        # Never cancel the task this is running in. A repair started from a
        # failing callback clears every binding, its own included, from inside
        # that binding's worker; cancelling it there would cut the repair off
        # at its next await. `_drain` ends such a worker once the binding is
        # gone.
    ...
    async def _drain(self, binding: _Binding) -> None:
        ...
        while True:
            ...
            if binding not in self._bindings:
                # Unbound while its callback ran -- a repair cleared it. The
                # repaired program has bound its own callbacks by now.
                return
