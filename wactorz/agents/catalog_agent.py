...
def _build_catalog() -> dict:
    ...
    if code:
        catalog["reachy-body"] = {
            ...
            "input_schema": {
                "cmd":         "str  — wake|sleep|pose|antennas|look_at|look_pixel|emotion|set_pose|bind|unbind|list_emotions|stop|say|volume",
                ...
                "gain_db":     "float — per-say dB below max (cmd=say); 0=loudest, negative=quieter",
                "loud":        "bool  — cmd=say; default true (compress+limit to max); false plays raw quiet TTS",
                "level":       "float — 0-100 persistent speech volume (cmd=volume); 100=loudest (default), lower=quieter",
                "db":          "float — persistent speech volume as dB below max (cmd=volume); 0=loudest, negative=quieter",
