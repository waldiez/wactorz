...
from wactorz.agents.llm.providers.openai import PLACEHOLDER_API_KEY
...
def test_a_custom_endpoint_needs_no_key(openai_calls: list[dict], missing_key: str | None) -> None:
    """`OPENAI_URL=https://litellm.local/v1` with `OPENAI_API_KEY=` left blank,
    as `.env.template` ships it. openai 2.x sent that empty key as no header;
    openai 3.x refuses to construct the client at all, which broke every keyless
    self-hosted endpoint on upgrade with an error demanding OPENAI_API_KEY."""
    OpenAIProvider(model="qwen3.5:4b", api_key=missing_key, base_url=_CUSTOM)
    assert openai_calls[0].get("base_url") == _CUSTOM
    assert openai_calls[0].get("api_key") == PLACEHOLDER_API_KEY


def test_a_custom_endpoint_keeps_the_key_it_was_given(openai_calls: list[dict]) -> None:
    """The placeholder answers a *missing* key only — a LiteLLM master key or
    a Groq key must reach the endpoint untouched."""
    OpenAIProvider(model="qwen3.5:4b", api_key="sk-litellm-master", base_url=_CUSTOM)
    assert openai_calls[0].get("api_key") == "sk-litellm-master"


@pytest.mark.parametrize("missing_key", ["", None])
def test_the_public_api_still_gets_no_invented_key(
    openai_calls: list[dict], missing_key: str | None
) -> None:
    """Without an endpoint override a missing key is a real mistake. The SDK's
    own error names the variable to set; a placeholder would turn that into a
    401 from api.openai.com that names nothing."""
    OpenAIProvider(model="gpt-4o", api_key=missing_key)
    assert openai_calls[0].get("api_key") == missing_key
