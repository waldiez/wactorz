...
impl WmeAgent {
    ...
    fn cmd_add(&self, parts: &[&str]) -> String {
        ...
        if !["movie", "show", "book", "podcast"].contains(&media_type.as_str()) {
            return format!("Unknown type `{media_type}`. Use: `movie`, `show`, `book`, or `podcast`.");
        }

        // Title may be quoted or bare multi-word
        let rest = parts[1..].join(" ");
        let (title, remainder) = if rest.starts_with('"') {
            if let Some(end) = rest[1..].find('"') {
                (rest[1..end + 1].to_string(), rest[end + 2..].trim().to_string())
            } else {
        ...
        let (progress, raw_rating, raw_genre): (Option<String>, Option<&str>, Option<&str>) =
            ...
            "show" => {
                let prog: Option<&str> = rem_parts.first().copied().filter(|s| {
                    s.to_lowercase().starts_with('s') && s.contains('e')
                });
        ...
        let rating: Option<f64> = raw_rating.and_then(|s: &str| {
            s.trim_end_matches('/').parse::<f64>().ok().filter(|&v| v >= 0.0 && v <= 10.0)
        });
        ...
        format!(
            "{icon} Logged **{title}**{rating_str}{progress_str}{genre_str2}{dequeued_note}"
        )
    }
    ...
    fn cmd_queue(&self, parts: &[&str]) -> String {
        match parts.first().copied().unwrap_or("list") {
            "add" => {
                ...
                let mut queue = self.queue.lock().unwrap();
                if queue.iter().any(|q| q.title.to_lowercase() == title.to_lowercase()) {
                    return format!("📋 **{title}** is already in the queue.");
                }
                queue.push(QueueItem { title: title.clone(), media_type: media_type.clone(), added_ms: Self::now_ms() });
                let icon = match media_type.as_str() { "movie" => "🎬", "show" => "📺", "book" => "📚", "podcast" => "🎙", _ => "🎭" };
                format!("{icon} Added **{title}** to queue ({} items total)", queue.len())
            }

            "list" | "ls" => {
                let queue = self.queue.lock().unwrap();
                if queue.is_empty() {
                    return "📭 Queue is empty.\n\nTry: `queue add movie \"Oppenheimer\"`".to_string();
                }
                let mut by_type: HashMap<&str, Vec<&str>> = HashMap::new();
                for q in queue.iter() {
                    by_type.entry(q.media_type.as_str()).or_default().push(q.title.as_str());
                }
                let mut sections = Vec::new();
                for (t, titles) in &by_type {
                    let icon = match *t { "movie" => "🎬", "show" => "📺", "book" => "📚", "podcast" => "🎙", _ => "🎭" };
                    let rows: Vec<String> = titles.iter().map(|t| format!("  • {t}")).collect();
                    sections.push(format!("{icon} **{}**\n{}", Self::capitalize(t), rows.join("\n")));
                }
                sections.sort();
                format!("**📋 Queue ({} items)**\n\n{}", queue.len(), sections.join("\n\n"))
            }
            ...
            "done" | "watched" | "read" | "finished" => {
                ...
                let rating: Option<f64> = parts.last()
                    .and_then(|s| s.parse::<f64>().ok())
                    .filter(|&v| v >= 0.0 && v <= 10.0);

                let mut queue = self.queue.lock().unwrap();
                let before = queue.len();
                let removed_type = queue.iter()
            ...
            sub => format!("Unknown queue sub-command `{sub}`. Use: `add`, `list`, `done`, `drop`."),
        }
    }
    ...
    fn cmd_stats(&self, filter: Option<&str>) -> String {
        ...
        for e in &entries {
            ...
            rec.0 += 1;
            if let Some(r) = e.rating { rec.1 += r; rec.2 += 1; }
        }

        let total = entries.len();
        let total_rated: u32 = by_type.values().map(|r| r.2).sum();
        let total_rating: f64 = by_type.values().map(|r| r.1).sum();
        let avg_overall = if total_rated > 0 { total_rating / total_rated as f64 } else { 0.0 };
        ...
        for t in type_order {
            if let Some(&(count, rs, rc)) = by_type.get(t) {
                let avg = if rc > 0 { format!("avg ⭐ {:.1}", rs / rc as f64) } else { "unrated".to_string() };
                let icon = match t { "movie" => "🎬", "show" => "📺", "book" => "📚", "podcast" => "🎙", _ => "🎭" };
                rows.push(format!("  {icon} **{}**: {count} entries — {avg}", Self::capitalize(t)));
            }
        }
        ...
        format!("{header}\n\n{}\n\n**Total entries**: {total}{avg_line}", rows.join("\n"))
    }

    fn cmd_top(&self, parts: &[&str]) -> String {
        let n: usize = parts.first()
            .and_then(|s| s.parse().ok())
            .unwrap_or(5)
            .min(20);
        let filter = parts.get(if parts.first().and_then(|s| s.parse::<usize>().ok()).is_some() { 1 } else { 0 })
            .copied();

        let log = self.log.lock().unwrap();
        let mut rated: Vec<&MediaEntry> = log.iter()
        ...
        rated.sort_by(|a, b| {
            b.rating.unwrap_or(0.0).partial_cmp(&a.rating.unwrap_or(0.0))
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let type_label = filter.map(|t| format!(" — {}", Self::capitalize(t))).unwrap_or_default();
        let rows: Vec<String> = rated.iter().take(n).enumerate().map(|(i, e)| {
            let icon = match e.media_type.as_str() { "movie" => "🎬", "show" => "📺", "book" => "📚", "podcast" => "🎙", _ => "🎭" };
            let genre = e.genre.as_deref().map(|g| format!(" `{g}`")).unwrap_or_default();
            format!("  {}. {icon} **{}** ⭐ {:.1}{genre}", i + 1, e.title, e.rating.unwrap_or(0.0))
        }).collect();
            ...
            "binge" => {
                ...
                if parts.len() < 3 {
                    return "Usage: `calc binge <episodes> <mins_per_ep>`\n\
                             ...
                             • `calc binge 5 13 45` → 5 seasons × 13 eps × 45 min".to_string();
                }
            ...
            "read" => {
                ...
                if parts.len() < 3 {
                    return "Usage: `calc read <pages> <pages_per_hour>`\n\n\
                             Example: `calc read 400 30`  → 400 pages @ 30 pp/h".to_string();
                }
                let pages: u64 = parts[1].parse().unwrap_or(0);
                let speed: u64 = parts[2].parse().unwrap_or(0);
                if pages == 0 || speed == 0 { return "Pages and speed must be positive.".to_string(); }
            ...
            "listen" => {
                ...
                if parts.len() < 3 {
                    return "Usage: `calc listen <episodes> <mins_per_episode>`\n\n\
                             Example: `calc listen 10 45`  → 10 episodes × 45 min".to_string();
                }
                let eps:  u64 = parts[1].parse().unwrap_or(0);
                let mins: u64 = parts[2].parse().unwrap_or(0);
                if eps == 0 || mins == 0 { return "Episodes and duration must be positive.".to_string(); }
