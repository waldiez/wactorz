...
| `WACTORZ_STT` | Speech recognition branch: `off` (default), `browser`, `server` or `host`. The browser reads it from `/api/config`, so one build serves deployments that differ. `server` is the branch the microphone button drives; `host` captures from this machine's own microphone instead, has no button (something must `POST /api/stt/listen`), routes what it hears as though typed, records it in the chat log, and needs `wactorz[host]` |
