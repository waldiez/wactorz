...
- Fixed: **other add-ons can no longer use Wactorz without a key.** Ports `8000` and `8888` are reachable from every add-on on Home Assistant's network, published or not, and with `api_key` blank anything there could spawn agents. The ingress shortcut was also accepted from that whole network rather than from the Supervisor alone. Now only the Supervisor's own address gets the shortcut, and with `api_key` blank the add-on generates a private key on first start and keeps it under `/data`. **The panel works exactly as before.** If you published port `8000` or `8888` and use it without a key, set `api_key` and send it as `X-API-Key` or `Authorization: Bearer`.
- Changed: the key and token options (`api_key`, `llm_api_key`, `mqtt_password`, `ha_token`, `discord_bot_token`, `telegram_bot_token`) are masked in the add-on's configuration screen. Their values are unchanged.
...
- Added: TLS for edge nodes, and the `mqtt_tls_ca` option. The add-on now creates its own certificate authority under `/data/state` and issues a broker certificate from it. `mosquitto_embedded` serves TLS on port `8883` — publish it for remote nodes — and for the official Mosquitto add-on Wactorz writes `wactorz-mqtt.crt` and `wactorz-mqtt.key` into `/ssl`, which the add-on now maps and where it writes nothing else: set that add-on's `certfile` and `keyfile` to them. `/deploy` switches a node to TLS only after checking from the node that the broker answers it, and `broker_tls` and `broker_tls_port` per deploy target override that. **A node uses TLS from its next `/deploy`**; Wactorz's own broker connection is unchanged.
- Changed: the embedded Mosquitto broker (`mosquitto_embedded`) saves its database every 5 minutes instead of every 30 seconds. It already saves whenever the add-on stops, restarts or updates, so this only changes what a crash or a power cut can lose, and it logs the "Saving in-memory database" line and rewrites the database on disk a tenth as often.
- Added: `node_accounts` option. A deployed node authenticates as itself instead of sharing this add-on's broker account, with a password derived for it rather than stored. With `mosquitto_embedded` it is on automatically, and that broker also loads an access list keeping each node to its own topics and the shared agent traffic. With the official Mosquitto add-on, turning it on writes `/share/wactorz/mosquitto-logins.yaml` for you to paste into that add-on's configuration -- accounts only, because its authentication plugin answers before any access list Wactorz could provide. **A node takes its account at its next `/deploy`**, so nothing changes until you redeploy; afterwards, change `mqtt_password` so the account they shared no longer opens anything.
