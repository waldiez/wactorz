...
impl WifAgent {
    ...
    fn cmd_add(&self, parts: &[&str]) -> String {
        if parts.is_empty() {
            return "Usage: `add <amount> [category] [note…]`\n\nExample: `add 25.50 food coffee`".to_string();
        }
        ...
        let expense = Expense { amount, category: category.clone(), note: note.clone(), ts_ms: Self::now_ms() };
        ...
        let budget_line = if let Some(&budget) = budgets.get(&category) {
            ...
            let icon = if pct >= 100.0 { "🔴" } else if pct >= 80.0 { "🟡" } else { "🟢" };
            format!(
        ...
        let note_part = if note.is_empty() { String::new() } else { format!(" _{note}_") };
    ...
    fn cmd_budget(&self, parts: &[&str]) -> String {
        ...
        let verb = if budgets.contains_key(&category) { "Updated" } else { "Set" };
        budgets.insert(category.clone(), amount);

        let expenses = self.expenses.lock().unwrap();
        let spent: f64 = expenses.iter().filter(|e| e.category == category).map(|e| e.amount).sum();
        let pct = if amount > 0.0 { spent / amount * 100.0 } else { 0.0 };
        let icon = if pct >= 100.0 { "🔴" } else if pct >= 80.0 { "🟡" } else { "🟢" };
    ...
    fn cmd_summary(&self, period: &str) -> String {
        ...
        if expenses.is_empty() {
            return "📭 No expenses recorded yet.\n\nTry: `add 25 food coffee` to get started.".to_string();
        }
        ...
        for e in &filtered { *by_cat.entry(e.category.clone()).or_default() += e.amount; }
        ...
        let rows: Vec<String> = cats.iter().map(|(cat, amt)| {
            let frac = if total > 0.0 { amt / total } else { 0.0 };
            let bar = Self::mini_bar(frac);
            let budget_note = budgets.get(cat).map(|&b| {
                let pct = amt / b * 100.0;
                let icon = if pct >= 100.0 { "🔴" } else if pct >= 80.0 { "🟡" } else { "🟢" };
    ...
    fn cmd_balance(&self) -> String {
        let budgets = self.budgets.lock().unwrap();
        if budgets.is_empty() {
            return "📋 No budgets set yet.\n\nTry: `budget food 300` then `add 25 food coffee`".to_string();
        }
        ...
        for cat in &sorted {
            ...
            let spent: f64 = expenses.iter()
                ...
                .map(|e| e.amount)
                .sum();
            let pct    = if budget > 0.0 { (spent / budget * 100.0).min(999.9) } else { 0.0 };
            ...
            rows.push(format!("{icon} **{cat}**: {bar} ${spent:.2} / ${budget:.2} ({pct:.0}%) — {rem_str}"));
            total_budget += budget;
            total_spent  += spent;
        }

        let overall_pct  = if total_budget > 0.0 { (total_spent / total_budget * 100.0).min(999.9) } else { 0.0 };
        let overall_icon = if overall_pct >= 100.0 { "🔴" } else if overall_pct >= 80.0 { "🟡" } else { "🟢" };
        rows.push(String::new());
        rows.push(format!("{overall_icon} **TOTAL**: ${total_spent:.2} / ${total_budget:.2} ({overall_pct:.0}%)"));
    ...
    fn cmd_calc(&self, parts: &[&str]) -> String {
        match parts.first().copied().unwrap_or("") {
            ...
            "compound" | "ci" => {
                ...
                if p <= 0.0 || t <= 0.0 { return "Principal and years must be positive.".to_string(); }
            ...
            "loan" | "mortgage" => {
                ...
                let p: f64 = parts[1].trim_start_matches('$').parse().unwrap_or(0.0);
                let r: f64 = parts[2].trim_end_matches('%').parse::<f64>().unwrap_or(0.0) / 100.0 / 12.0;
                let n: f64 = parts[3].parse::<f64>().unwrap_or(0.0) * 12.0;
                if p <= 0.0 || n <= 0.0 { return "Principal and years must be positive.".to_string(); }
            ...
            "roi" => {
                if parts.len() < 3 {
                    return "Usage: `calc roi <initial> <final>`\n\nExample: `calc roi 5000 7500`".to_string();
                }
    ...
    fn cmd_tips(&self, topic: &str) -> String {
        match topic {
            ...
            _ => {
                "**WIF Financial Tips** — pick a topic:\n\n\
                 ```\n\
