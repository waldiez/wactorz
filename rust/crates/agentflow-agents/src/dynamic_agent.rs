...
impl Actor for DynamicAgent {
    ...
    async fn handle_message(&mut self, message: Message) -> Result<()> {
        ...
        let content = match &message.payload {
            agentflow_core::message::MessageType::Text { content } => content.clone(),
            agentflow_core::message::MessageType::Task { description, .. } => description.clone(),
            _ => return Ok(()),
        };
        ...
        for log in &logs {
            ...
            if let Some(pub_) = &self.publisher {
                pub_.publish(
                    agentflow_mqtt::topics::logs(&self.config.id),
    ...
    async fn on_stop(&mut self) -> Result<()> {
        if let Some(pub_) = &self.publisher {
            pub_.publish(
                agentflow_mqtt::topics::status(&self.config.id),
                &serde_json::json!({
                    "agentId":   self.config.id,
                    "agentName": self.config.name,
                    "state":     "stopped",
                ...
                msg = rx.recv() => {
                    match msg {
                        ...
                        Some(m) => {
                            ...
                            use agentflow_core::message::{ActorCommand, MessageType};
                            match &m.payload {
