...
- **A program a node repaired stays repaired.** An agent whose code fails can be fixed by the LLM in place, and the fix is written back to the supervisor so the next restart runs it. On a node the supervisor was unreachable from the agent, so the fix lived only in memory: every restart rebuilt the original broken program and paid to repair it again. Its new code now reaches the restart. **Known limit:** it still does not outlive the node's process — the spawn registry that would carry it across a reboot lives on main, and a node has no way to tell main it repaired something. Until it does, a restarted node runs the code it was last sent.
