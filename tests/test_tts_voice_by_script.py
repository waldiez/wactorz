...
takes much longer doing it. Hence the swap to a Greek voice.
...
class TestTheGuideAndTheCodeAgree:
    def test_the_guide_documents_the_greek_override(self) -> None:
        text = Path("docs/catalogue-reachy-mini.md").read_text(encoding="utf-8")

        assert "TTS_VOICE_EL" in text
        assert "el-GR-NestorasNeural" in text, (
            "the guide should name the configurable alternative so the choice "
