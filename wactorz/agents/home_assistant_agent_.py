...
class HomeAssistantAgent(LLMAgent):
    ...
    async def _process(self, text: str) -> dict[str, Any]:
        ...
        if action == "create_automation":
            ...
            return await self._create_automation(
                text, entities, hardware_result.get("hardware", [])
            )
    ...
    async def _classify_action(self, text: str) -> str:
        ...
        try:
            ...
            word = (
                (response or "").strip().lower().split()[0]
            ...
            logger.warning(
                "[%s] Action classification LLM call failed: %s", self.name, exc
            )
    ...
    def _classify_action_heuristic(text: str) -> str:
        ...
        if any(
            w in lower
            for w in ("list areas", "show areas", "show me areas", "what areas")
        ):
            return "list_areas"
        if any(
            w in lower
            for w in ("list devices", "show devices", "show me devices", "what devices")
        ):
            return "list_devices"
        if any(
            w in lower
        ...
        if any(
            w in lower for w in ("delete", "remove automation", "disable automation")
        ):
    ...
    async def _get_devices(self) -> dict[str, Any]:
        ...
        if (
            cached is not None
            and now - float(self._device_cache.get("timestamp", 0.0))
            < self._device_cache_ttl
        ):
            return cached
        ...
        try:
            devices = await fetch_devices_entities_with_location(
                self.ha_url, self.ha_token, include_states=True
            )
    ...
    async def _select_hardware(
        self, text: str, devices: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return self._format_hardware_result(
                text, devices, selected, can_fulfill, fallback_text
            )

        except Exception as exc:
            logger.error(
                "[%s] Hardware selection failed: %s", self.name, exc, exc_info=True
            )
            return self._format_hardware_result(
                text, devices, [], False, f"Hardware selection error: {exc}"
            )

    async def _recommend_hardware(
        self, text: str, devices: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "[%s] Received hardware recommendation response from LLM.", self.name
            )
            self._accumulate_usage(usage)
            data = json.loads(self._strip_fences(response))
            if not isinstance(data, dict):
            ...
            if can_fulfill and not primary:
                ...
                if isinstance(retry_data, dict):
                    ...
                    alternatives = self._filter_hardware_alternatives(
            ...
            logger.error(
                "[%s] Hardware recommendation failed: %s", self.name, exc, exc_info=True
            )
            return self._format_available_hardware_result(
                text,
                devices,
    ...
    def _format_hardware_result(
        ...
        if connected:
            ...
            lines.append(
                f"Device discovery unavailable: {devices.get('reason', 'N/A')}."
            )
    ...
    async def _create_automation(
        ...
        try:
            ...
            logger.error(
                "[%s] Automation creation failed: %s", self.name, exc, exc_info=True
            )
            return {
                "can_create": False,
                "inserted": False,
    ...
    async def _generate_automation(
        ...
        if not can_create:
            return {
                "can_create": False,
                "result": result_text
                or "Not enough information to safely create an automation.",
                "automation": {},
            }
        ...
        return {
            ...
            "automation": {
                ...
                "description": automation.get(
                    "description", "Generated by home-assistant-agent"
                ),
    ...
    async def _insert_automation(self, automation: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            response = await create_automation_via_rest(
                self.ha_url, self.ha_token, automation
            )
    ...
    def _list_automations(self, automations: list[dict[str, Any]]) -> dict[str, Any]:
        if not automations:
            suffix = (
                " (or Home Assistant is not configured)." if not self.ha_url else "."
            )
            return {
                "result": f"No automations found in Home Assistant{suffix}",
                "automations": [],
            }
    ...
    async def _fetch_registry_items(
        self, fetcher: Any
        ...
        try:
            ...
            logger.warning(
                "[%s] Could not fetch Home Assistant registry data: %s", self.name, exc
            )
    ...
    async def _list_devices(self) -> dict[str, Any]:
        ...
        for idx, row in enumerate(device_rows, 1):
            details = " ".join(
                p for p in (row["manufacturer"], row["model"]) if p
            ).strip()
            if details:
    ...
    async def _delete_automation(
        ...
        if not automations:
            return {
                "result": "No automations found in Home Assistant to delete.",
                "deleted": False,
            }
        if self.llm is None:
        ...
        try:
            ...
            return {
                "result": f"Could not identify automation to delete: {exc}",
                "deleted": False,
            }
        ...
        if not isinstance(data, dict) or not data.get("found"):
            return {
                "result": str(
                    data.get("result", "Could not identify which automation to delete.")
                ),
                "deleted": False,
            }
        ...
        if not automation_id:
            return {
                "result": "Could not determine automation ID to delete.",
                "deleted": False,
            }
        if not self.ha_url or not self.ha_token:
    ...
    async def _edit_automation(
        ...
        if not automations:
            return {
                "result": "No automations found in Home Assistant to edit.",
                "edited": False,
            }
        if self.llm is None:
        ...
        try:
            ...
            return {
                "result": f"Could not identify automation to edit: {exc}",
                "edited": False,
            }
        ...
        if not isinstance(ident_data, dict) or not ident_data.get("found"):
            return {
                "result": str(
                    ident_data.get(
                        "result", "Could not identify which automation to edit."
                    )
                ),
                "edited": False,
            }
        ...
        if not automation_id:
            return {
                "result": "Could not determine the automation ID to edit.",
                "edited": False,
            }
        ...
        if self.ha_url and self.ha_token:
            try:
                ...
                match = next(
                    (
                        ...
                        and (
                            ...
                            or a.get("alias") == automation_name
                        )
                    ),
                    None,
                )
                ...
                logger.warning(
                    "[%s] Could not fetch full automation config: %s", self.name, exc
                )
        ...
        try:
            ...
            return {
                "result": f"LLM could not generate updated automation: {exc}",
                "edited": False,
            }
        ...
        if error:
            return {
                "result": f"Updated automation is invalid: {error}",
                "edited": False,
            }
        ...
        try:
            await update_automation(
                self.ha_url, self.ha_token, automation_id, updated_automation
            )
            self._automation_cache = {"timestamp": 0.0, "data": None}  # invalidate
            return {
    ...
    def _validate_automation(automation: dict[str, Any]) -> str | None:
        if (
            not isinstance(automation.get("name"), str)
            or not automation["name"].strip()
        ):
    ...
    def _filter_hardware_alternatives(
        ...
        for item in alternatives:
            ...
            if (
                ...
                continue
            identity = (
                str(item.get("hardware", "")).strip().lower(),
                tuple(entity_ids),
            )
            if identity in seen:
                continue
