...
- **The `mqtt_ws_port` add-on option, and the broker's WebSocket listener.** The listener existed for a browser MQTT client the dashboard no longer has — real-time updates arrive over the monitor's own WebSocket — and the port value was read into configuration and then never used. Both are gone, along with `--mqtt-ws-port` on `wactorz-monitor`. Existing add-on installations may log a one-time "not in schema" notice for the removed option until their configuration is saved again.
