...
- **Pausing an agent now stops it answering chat.** Pause suspended the agent's message queue, but messages sent from the chat view reach an agent directly rather than through that queue — so a paused agent kept replying as though nothing had happened. It now declines until resumed. Pausing, resuming, stopping and deleting also behave the same whichever route they arrive by: the dashboard, chat commands, the REST interface, or another agent. Two of those routes previously skipped telling the supervisor that the stop was deliberate, so the agent was restarted moments later, and neither respected protected agents.
