.PHONY: help dev dev-full dev-backend precommit-install precommit-run build build-frontend fmt lint clean \
        up down logs shell \
...
fmt: ## Format TypeScript (frontend)
	cd $(FRONTEND_DIR) && $(PKG_MGR) run fmt 2>/dev/null || $(PKG_MGR) x prettier --write "src/**/*.ts"

format: fmt ## Format TypeScript (frontend)

lint: ## TS typecheck (frontend)
