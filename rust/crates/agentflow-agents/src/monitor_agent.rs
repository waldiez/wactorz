...
use agentflow_core::{Actor, ActorConfig, ActorMetrics, ActorState, ActorSystem, EventPublisher, Message};
...
impl MonitorAgent {
    ...
    async fn publish_health(&self, reports: &[ActorHealthReport]) {
        ...
        for report in &stale {
            pub_.publish(
                agentflow_mqtt::topics::alert(&report.actor_id),
                &serde_json::json!({
        ...
        pub_.publish(
            agentflow_mqtt::topics::SYSTEM_HEALTH,
            &serde_json::json!({
...
impl Actor for MonitorAgent {
    ...
    async fn handle_message(&mut self, message: Message) -> Result<()> {
        ...
        if let MessageType::Command {
            command: agentflow_core::message::ActorCommand::Status,
        } = &message.payload
        {
