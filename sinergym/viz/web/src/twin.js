...
import { ArcRotateCamera, Color3, Color4, Engine, GlowLayer, HemisphericLight, Mesh, MeshBuilder, Scene, StandardMaterial, Vector3, VertexData, } from "@babylonjs/core";
...
export class Twin {
    ...
    zoneMat = new Map();
    floorMeshes = new Map();
    target = new Map(); // smoothed colour target
    constructor(canvas, geo) {
        ...
        const ep = (v) => new Vector3(v[0] - cx, v[2] * VSCALE, v[1] - cy);
        this.camera = new ArcRotateCamera("cam", Math.PI * 0.85, Math.PI * 0.36, span * 1.7, new Vector3(0, topY * 0.45, 0), this.scene);
        ...
        for (const [name, z] of Object.entries(geo.zones)) {
            ...
            arr.push(mesh);
            this.floorMeshes.set(z.floor, arr);
            if (z.occupied)
    ...
    setZoneTemp(zone, temp) {
        if (!this.zoneMat.has(zone))
            return;
        this.target.set(zone, tempColor(temp));
    }
    flashZone(zone) {
        const mat = this.zoneMat.get(zone);
        if (!mat)
            return;
        let t = 0;
    ...
    setFloorVisible(floor, visible) {
        for (const m of this.floorMeshes.get(floor) ?? [])
