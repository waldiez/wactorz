...
- **Wactorz can listen and answer aloud, and where each of those happens is a setting rather than a build.** `WACTORZ_STT` and `WACTORZ_TTS` each name a branch -- `off`, `browser`, `server` or `host` -- and `WACTORZ_STT_URI` / `WACTORZ_TTS_URI` name the services behind them, with the scheme choosing the kind: `tcp://` is a Wyoming recogniser or synthesiser, `ws://` is a [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) streaming recogniser, and `http(s)://` is an endpoint that takes text and answers with audio. A deployment that names none of them is unaffected, and the browser learns the branch from `/api/config` rather than from how the bundle was built, so one wheel serves installs that differ.
  ...
  **Words now appear in the composer while you are still speaking**, when the recogniser streams. The audio rides the dashboard's existing WebSocket rather than a second connection, corrections replace what was heard rather than repeating it, and typing while the microphone is open survives the next reading -- an edit on either side of the words is kept, and a reading you delete stays deleted. Capture runs on the browser's audio thread, so a page busy drawing a reply delays frames instead of losing them.

  **Speech can be made somewhere other than this process.** `browser` uses the browser's own voice, so the text is sent nowhere at all -- the setting for a deployment that must not hand what it says to a synthesiser. `server` makes it here, by the built-in voice or by whatever `WACTORZ_TTS_URI` names, and plays it in the page. Measurements are spelled out on the way, because a self-hosted synthesiser reads `21 °C` with no degrees in it and `5 m/s` with a "slash".

  **`browser` listens too, where the browser can.** The recogniser the browser carries needs nothing of this deployment, so no audio reaches it -- though that is not the same as the audio staying on the machine, since Chrome sends it to Google and Safari to Apple. Only Chromium implements it, and only on a page trusted with a microphone, so the button is offered over localhost or TLS and nowhere else. A deployment that must keep speech inside its own network wants `server` with a self-hosted recogniser.

  **Which branch listens and which speaks can be changed while running**, from Settings, without editing a file or restarting: the environment sets what a deployment starts as, and a choice made in the dashboard supersedes it. Choices are kept, so a restart comes back to what was chosen rather than to what was configured, and "Reset to configured" is the way back. The voice can be chosen the same way. The service addresses cannot -- they name services this process opens connections to, and one a browser can write is one it can point at anything reachable from the machine.

  **`host` is the branch with no page.** It captures from the machine's own microphone and answers through its own speakers, for a room rather than a screen. It has no button: something asks it to listen by posting to `/api/stt/listen`, and it records until whoever is speaking stops -- judged against the room's own noise, because a quiet laptop microphone and a desk beside a fan are nothing alike. What it hears is routed exactly as though typed, and both the question and the answer are written to the chat log and shown to any browser that is open. Only replies to a turn someone started are read aloud, so agents talking among themselves stay quiet, and **it will not listen while it is speaking**, so a room does not hear its own reply and answer it. Settings offers **Test microphone**, which records one turn and shows what was heard without routing it -- asking whether the device works is a hardware question, and answering it through a model would spend a turn and leave a row in the chat log. The voice this deployment speaks in is chosen there too, which the picker in the audio popover cannot do: that one is this browser's own, and on `host` nobody is at a browser.

  **A phrase can start a turn, so a room needs no button at all.** `WACTORZ_WAKE=on` has `host` listen for what `WACTORZ_WAKE_WORDS` names -- given as words rather than a trained model, so an invented name costs no more than a common one -- and a turn begins when it hears one. The turn is read from the same open microphone the phrase arrived on, because a sentence begins in the same breath as the wake word and closing the device to reopen it loses the start of every one. What it hears is routed exactly as though typed and marked with where it came from, so a turn a room started can be told from one somebody typed. It is switched on and off in Settings like the branches are, and the microphone check borrows the device rather than waiting for the phrase: the people who most need to know whether a microphone works are the ones whose wake word is not being heard.

  `infra/voice/stt/` builds a streaming recogniser that needs no GPU. **The optional dependencies are now named for what they do**: `wactorz[stt]` to recognise speech, `wactorz[tts]` to make it, `wactorz[host]` to use this machine's own microphone and speakers, `wactorz[wake]` to wait for a spoken phrase, and `wactorz[voice]` for all of them -- which installs everything it did before, so nothing needs changing.
