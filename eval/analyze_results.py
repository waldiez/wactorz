...
def table(rows, title, keys=("model", "category")):
    ...
    print(
        f"{'key':<{width}} {'n':>4} {'err':>4} {'acc':>7} {'lat(s)':>8} "
        f"{'out_tok':>8} {'tok/s':>7} {'cost($)':>9}"
    )
    print("-" * (width + 54))
    for r in rows:
        key = " / ".join(str(r[k]) for k in keys)
        lat = f"{r['lat']:.2f}" if r["lat"] is not None else "-"
        tok = f"{r['out_tok']:.0f}" if r["out_tok"] is not None else "-"
        tps = f"{r['tok_s']:.0f}" if r["tok_s"] is not None else "-"
        print(
            f"{key:<{width}} {r['n']:>4} {r['err']:>4} {r['acc']:>6.1%} {lat:>8} "
            f"{tok:>8} {tps:>7} {r['cost']:>9.4f}"
        )
...
def summarise(records, keyfn):
    ...
    for key, rs in sorted(buckets.items()):
        ...
        # Raw latency is not comparable across a local model and a remote API:
        # it carries network RTT and, more importantly, however many tokens the
        # model chose to emit. Generation throughput is the honest number, so
        # aggregate tokens over aggregate time rather than averaging per-call
        # rates (which lets one short reply dominate).
        toks = [r.get("output_tokens") or 0 for r in oks]
        billable = [(t, r["latency_s"]) for t, r in zip(toks, oks) if t and r.get("latency_s")]
        rows.append(
            {
                "key": key,
                "n": len(rs),
                ...
                "out_tok": sum(toks) / len(oks) if oks else None,
                "tok_s": (
                    sum(t for t, _ in billable) / sum(s for _, s in billable) if billable else None
                ),
...
def main() -> int:
    ...
    if args.csv:
        ...
        with open(args.csv, "w", newline="", encoding="utf-8") as fh:
            w = _csv.DictWriter(
                fh,
                fieldnames=["model", "category", "n", "err", "acc", "lat", "out_tok", "tok_s", "cost"],
            )
            w.writeheader()
            for row in by_cat:
