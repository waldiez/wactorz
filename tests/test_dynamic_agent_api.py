...
class TestTheSyncAsyncContract:
    ...
    def test_what_the_planner_strips_await_from_is_really_synchronous(self, method: str) -> None:
        ...
        """
        fn = getattr(AgentAPI, method, None)
    ...
    def test_the_calls_documented_as_async_still_are(self, method: str) -> None:
        ...
        assert inspect.iscoroutinefunction(getattr(AgentAPI, method))


class TestIdentity:
    def test_the_api_reports_the_actor_name(self, api: AgentAPI) -> None:
        assert api.name == "probe"

    def test_the_api_reports_the_actor_id(self, api: AgentAPI, tmp_path: Path) -> None:
        assert api.actor_id

    def test_node_is_a_string_even_when_unset(self, api: AgentAPI) -> None:
        """Generated code interpolates this into topics, so it cannot be None."""
        assert isinstance(api.node, str)


class TestStateRoundTrip:
    def test_what_is_persisted_can_be_recalled(self, api: AgentAPI) -> None:
        api.persist("last_seen", {"value": 42})

        assert api.recall("last_seen") == {"value": 42}

    def test_recalling_something_never_stored_gives_none(self, api: AgentAPI) -> None:
        assert api.recall("never_written") is None

    def test_recall_accepts_a_default(self, api: AgentAPI) -> None:
        assert api.recall("never_written", "fallback") == "fallback"

    def test_state_is_a_mapping_the_code_can_use(self, api: AgentAPI) -> None:
        api.state["counter"] = 1
...
class TestDiscovery:
    def test_agents_returns_a_list_without_a_registry(self, api: AgentAPI) -> None:
        """Generated code iterates the result, so it must never be None."""
        assert isinstance(api.agents(), list)

    def test_topics_returns_a_list_without_a_bus(self, api: AgentAPI) -> None:
