...
- **An agent migrated to a node no longer loses everything it learns there the next time that node reboots.** The state snapshot handed over during a migration is applied by taking the agent's state file away and writing the snapshot in its place — right on arrival, because the source held the authoritative copy at that moment, and wrong every time afterwards, because the agent has been living and accumulating on the target ever since. The snapshot was kept in both of the records the node reads back: the spawn registry and the node's retained desired state. So a reboot reconciled the agent, found the snapshot still attached, and rolled it back to the moment it arrived — conversation history, counters and calibration alike — with more lost the longer the agent had been there. Those two records now describe what the agent *is*, and the snapshot rides only on the spawn message it belongs to, which the runner already clears once it has been consumed. Upgrading is enough to fix an agent already in this state: a snapshot left in the registry by an older version is dropped as the entry is read, so the next desired state published for that node no longer carries it.
